...
public:
	...
	void tannlinfit(double* x0, double* y0, Te a_init,double* &as)
	{
		const int N = _msize(x0) / sizeof(*x0);
		//double a_init[6] = { ainit[0],ainit[1],ainit[2],ainit[3],ainit[4],ainit[5] };
		//int leng = _msize(a_init) / sizeof(*a_init);
		//int leng = sizeof(a_init) / sizeof(a_init[0]);
		...
		double w1 = 0.8;
		double slope2 = 7;
		//double x_init[6] = { x_init1[0],x_init1[1],x_init1[2] ,x_init1[3] ,x_init1[4] ,x_init1[5] };
		//double x_init[6] = { h / 2, h / 2, slope1 ,ped_pos,w1,slope2 };
		//double x_init[6] = { 0.5, 0.5, 0.05 ,0.8,0.09,-0.07 }; /* starting values */
		//double x_init[5] = { h / 2, h / 2, slope1 ,ped_pos,w1};
		gsl_vector_view x = gsl_vector_view_array(a_init, p);
		...
		status = gsl_multifit_nlinear_driver(3000, xtol, gtol, ftol, NULL
			, NULL, &info, w);
		//std::cout << "----" << std::endl;
	...
	template <typename Te>
	double* fitFirsttoGetinitda(double* x0, double* y0,Te x_init)
	{
		
		//tannlinfit(x0, y0,x_init);
		//double ai0[6] = {as[0],as[1],as[2],as[3],as[4],as[5]};
		//return as;
	}
	template <typename Te>
	void refl_fitting(double* x0, double* y0, double*& fitx, double*& fity, double* &as,Te x_init,int nx)
	{
		//const int N = _msize(x0) / sizeof(*x0);
		//std::cout << N;
		tannlinfit(x0, y0,x_init,as);
		const int PointNums = nx;
		double x00 = 0;//2.12
		//double* x = new double[PointNums];
		//double* y = new double[PointNums];
		
		const double interdis = (1 - x00) / (PointNums-1);//2.35
